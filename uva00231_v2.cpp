...
int main() {
    ...
    while(1) {
        ...
        int n = vec.size();
        _vec<int> vec_l;
        vec_l.push_back(vec[0]);
        for(int i=1; i<n; ++i) {
            int pos = upper_bound(vec_l.begin(), vec_l.end(), vec[i], greater<int>()) - vec_l.begin();
            if(pos==vec_l.size()) vec_l.push_back(vec[i]);
            else vec_l[pos] = vec[i];
        }

        cout<<"  maximum possible interceptions: "<<vec_l.size()<<"\n";
    }
