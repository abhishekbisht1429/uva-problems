...
_vec<_vec<int>> validPos;

bool place(_vec<int> &row, int i, int q) {
    for(int j=0; j<q; ++j) {
        if(row[j] == i || abs(q - j) == abs(i - row[j]))
            return false;
    }
    return true;
}
void findValidPos(_vec<int> &row, int q=0) {
    if(q>7) {
        validPos.push_back(row);
        return;
    }
...
int main() {
    ...
    while(1) {
        ...
        for(auto validRow : validPos) {
            int moves = 0;
            for(int j=0; j<8; ++j) {
                moves += (validRow[j] - row[j]) != 0;
            }
            min_moves = min(min_moves, moves);
        }
        cout<<"Case "<<(++tc)<<": "<<min_moves<<"\n";
    }
