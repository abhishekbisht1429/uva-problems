...
void bfs(_vec<_vec<char>> &grid, int si, int sj) {
    queue<_ii> q;
    q.push({si, sj});
    _vec<int> di = {0, 0, -1, -1, -1, 1, 1, 1};
    _vec<int> dj = {1, -1, 0, -1, 1, 0, -1, 1};
    while(!q.empty()) {
        auto p = q.front();
        q.pop();
        if(grid[p.first][p.second] == '@') {
            for(int k=0; k<8; ++k) {
                ...
                if(ni >= 0 && ni < grid.size() && nj >= 0 && nj < grid[0].size())
                    q.push({p.first + di[k], p.second + dj[k]});
            }
...
int main() {
    ...
    while(1) {
        ...
        if(m==0)
            break;
        _vec<_vec<char>> grid(m, _vec<char>(n));
