...
int main() {
    ...
    while(1) {
        ...
        _vec<_vec<_ll>> cum_mat(n, _vec<_ll>(n));
        for(int i=0; i<n; ++i) {
            for(int j=0; j<n; ++j) {
                cin>>cum_mat[i][j];

                if(i>0) cum_mat[i][j] += cum_mat[i-1][j];
                if(j>0) cum_mat[i][j] += cum_mat[i][j-1];
                if(i>0 && j>0) cum_mat[i][j] -= cum_mat[i-1][j-1];
            }
        }
        ...
        for(int i1=0; i1+m-1<n; ++i1) {
            for(int j1=0; j1+m-1<n; ++j1) {
                int i2 = i1+m-1, j2 = j1+m-1;

                _ll sum = cum_mat[i2][j2];
                if(i1>0) sum -= cum_mat[i1-1][j2];
                if(j1>0) sum -= cum_mat[i2][j1-1];
                if(i1>0 && j1>0) sum += cum_mat[i1-1][j1-1];
