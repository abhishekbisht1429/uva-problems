...
bool valid(_vec<int> vec, _vec<int> &swapMap) {
    for(int i=0; i<swapMap.size(); ++i) {
        swap(vec[swapMap[i]], vec[swapMap[i]+1]);
    }
...
int minSwapMaps(_vec<int> vec, _vec<int> &swapMap, int i=0) {
    if(i == swapMap.size()) {
        if(i > 0 && valid(vec, swapMap)) {
            return 1;
        }
        return 0;
    }
    ...
    for(int j=0; j<vec.size()-1; ++j) {
        swapMap[i] = j;
        count += minSwapMaps(vec, swapMap, i+1);
    }
    return count;
}
int main() {
    ...
    while(1) {
        ...
        _vec<int> swap_map(min_swaps);
        cout<<"There are "<<minSwapMaps(vec, swap_map)<<" swap maps for input data set "<<(++tc)<<".\n";
