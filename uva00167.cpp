...
bool place(_vec<int> &col, int q, int j) {
    for(int i=0; i<q; ++i) {
        if(col[i] == j || abs(q-i) == abs(j - col[i])) 
            return false; 
    }
    return true;
}
_p<int, bool> maxScore(_vec<int> &col, _vec<_vec<int>> &score, int q=0) {
    if(q>7) return {0, true};
    ...
    for(int j=0; j<8; ++j) {
        if(place(col, q, j)) {
            col[q] = j;
            auto p = maxScore(col, score, q+1);
            if(p.second) {
                valid = true;
                max_score = max(max_score, score[q][j] + p.first);
            }
        }
    }
    return {max_score, valid};
}
int main() {
    ...
    cin>>tc;
    while(tc--) {
        _vec<_vec<int>> score(8, _vec<int>(8));
        for(int i=0; i<8; ++i) {
            for(int j=0; j<8; ++j) {
                cin>>score[i][j];
            }
        ...
        _vec<int> temp(8);
        cout<<setw(5)<<maxScore(temp, score).first<<"\n";
