//
// Created by reckoner1429 on 12/01/22.
//
...
_vec<_vec<int>> dp(1005, _vec<int>(35));
_vec<int> weights;
_vec<int> values;
int maxVal(int remN, int remW) {
    if(remW <= 0 || remN <= 0)
        return 0;
    if(dp[remN][remW] == -1)
        dp[remN][remW] = max(maxVal(remN-1, remW),
                             maxVal(remN-1, remW - weights[remN-1])
                             + ((remW >= weights[remN-1])?values[remN - 1]:0)) ;
    return dp[remN][remW];
}
