...
int main() {
    ...
    while(tc--) {
        int n;
        cin>>n;
        _vec<_vec<int>> torus(n, _vec<int>(n));
        for(int i=0; i<n; ++i) {
            for(int j=0; j<n; ++j) {
                cin>>torus[i][j];
            }
        ...
        for(int i1=0; i1<n; ++i1) {
            squash.assign(n, 0);
            for(int i2=i1; i2<n || (i2%n)<i1; ++i2) {
                for(int j=0; j<n; ++j) 
                    squash[j] += torus[i2%n][j];
                
                int total = accumulate(squash.begin(), squash.end(), 0);
                int sum = 0, max_squash_sum = INT_MIN;
                for(int j=0; j<n; ++j) {
                    sum += squash[j];
                    max_squash_sum = max(max_squash_sum, sum);
                    sum = max(0, sum);
                }
                sum = 0;
                for(int j=0; j<n; ++j) {
                    sum += squash[j];
                    max_squash_sum = max(max_squash_sum, total - sum);
                    sum = min(0, sum);
                }
                
                max_s = max(max_s, max_squash_sum);
            }
        }
