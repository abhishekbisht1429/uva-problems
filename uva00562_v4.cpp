...
int main() {
    ...
    while(tc--) {
        ...
        dp.resize(total/2+1);
        dp.assign(total/2+1, 0);

        /* Bottom Up Approach with memory saving trick */
        /* Keep the order of loops in mind */
        for(int i=0; i<m; ++i)
            for(int s=total/2; s>0; --s)
                if(s >= coins[i]) dp[s] = max(dp[s], dp[s - coins[i]] + coins[i]);

        cout<<abs(total - 2*dp[total/2])<<"\n";
    }
