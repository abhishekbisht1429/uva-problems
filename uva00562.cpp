...
int findOptSol(int s=0, int i=0) {
    if(i==m)
        return s;

    if(dp[s][i] == -1) {
        int v1 = findOptSol(s, i+1);
        int v2 = findOptSol(s+coins[i], i+1);
        if(abs(total - 2*v1) < abs(total - 2*v2))
            dp[s][i] = v1;
        else
            dp[s][i] = v2;
    }
...
int main() {
    ...
    while(tc--) {
        ...
        dp.resize(total+1);
        dp.assign(total+1, _vec<int>(m, -1));

        cout<<abs(total - 2*findOptSol())<<"\n";
    }
