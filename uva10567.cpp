...
int main() {
    ...
    #endif
    /* ######################CODE_START################################ */
    _umap<char, _vec<int>> pos_map;
    string inp;
    cin>>inp;
    for(int i=0; i<inp.size(); ++i) {
        if(pos_map.find(inp[i])==pos_map.end())
            pos_map[inp[i]] = _vec<int>();
        pos_map[inp[i]].push_back(i);
    }
    ...
    while(q--) {
        ...
        for(int i=0; i<str.size(); ++i) {
            char c = str[i];
            auto itr = upper_bound(pos_map[c].begin(), pos_map[c].end(), pos);
            if(itr == pos_map[c].end()) {
                matched = false;
                break;
            } else {
                pos = *itr;
