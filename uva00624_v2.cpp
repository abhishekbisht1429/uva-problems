...
/* NOTE: Even though the solution below is iterative, it takes more time as 
 *       checks all the 2^t combinations.
 *       The previous version of the solution using backtracking 
 *       takes less time for the input test cases given at uva online judge
*/
int main() {
        ...
        for(_uint s=0; s<(1<<t); ++s) {
            ...
            if(sum <= n) {
                if(sum > opt_sum) {
                    opt_s = s;
                    opt_sum = sum;
                } else if(sum == opt_sum && __builtin_popcount(s) > __builtin_popcount(opt_s)) {
                    opt_s = s;
                }
            }
        }
        for(int i=0; i<t; ++i) {
            if(_isSet(opt_s, i)) cout<<tracks[i]<<" ";
        }
        cout<<"sum:"<<opt_sum<<"\n";
    }
