...
_vec<int> opt_selection;
int opt_sum = 0;
void findOptSol(_vec<int> &tracks, _vec<int> &selected, int n, int sum=0, int i=0) {
    if(sum > n) return;
    if(i==tracks.size()) {
        if(sum > opt_sum) {
            opt_selection = selected;
            opt_sum = sum;
        } else if(sum == opt_sum && selected.size() > opt_selection.size()) {
            opt_selection = selected;
        }
        return;
    }

    findOptSol(tracks, selected, n, sum, i+1);
...
int main() {
    ...
    while(1) {
        ...
        _vec<int> temp;
        findOptSol(tracks, temp, n);
        
        for(int i : opt_selection) {
            cout<<i<<" ";
        }
        cout<<"sum:"<<opt_sum<<"\n";
    }
