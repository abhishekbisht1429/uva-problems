...
int main() {
    ...
    while(tc--) {
        ...
        while(1) {
            getline(cin, inp);
            if(cin.fail() || inp.size() == 0) break;

            mat.push_back(_vec<int>(inp.size()));
        ...
        int max_area = 0;
        _vec<int> squash_vec(n);
        for(int i1=0; i1<n; ++i1) {
            squash_vec.assign(n, 0);
            for(int i2=i1; i2<n; ++i2) {
                for(int j=0; j<n; ++j)
                    squash_vec[j] += mat[i2][j];

                int height = i2-i1+1;
                int max_len = 0, len = 0;
                for(int j=0; j<n; ++j) {
                    if(squash_vec[j] == height) {
                        len++;
                ...
                max_area = max(max_area, max_len*height);
            }
        }
